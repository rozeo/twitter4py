...
class gekitotsuChecker:
    ...
    def CheckGekitotsu(self,jq):
        ...
        if re.search(self.gekitotsu,jq['text']) and self.ts_ms > 0:
            ...
            message = "@%s 追突するまでに %.4f秒 かかりました" % (jq['user']['screen_name'],(float(gmst - self.ts_ms) / 1000.0))
        
            t4p.request("POST","statuses/update",{"status" : message,"in_reply_to_status_id" : jq['id']})
